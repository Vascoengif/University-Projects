...
int marcar(int sz, int tabuleiro[][sz], int x, int y){
    ...
    if(x >= 0 && y >= 0 && x < sz && y < sz){
        ...
        // Verifica se tem peça igual no topo
...
void gravidade(int sz, int tabuleiro[][sz]){
    ...
    for(j = 0; j < sz; j++) {
        ...
        for(i = 0; i < (sz-1); i++){
            ...
            if(tabuleiro[i][j] == -1 && tabuleiro[i+1][j] != -1) {                 //descemos no normal e subimos no tabuleiro de jogo
...
void coluna(int sz, int tabuleiro[][sz]){
	...
	for(j = 0; j < (sz-1); j++){                                                      //  for(j = 0; j < (sz-1); j++){
		if(tabuleiro[0][j] == -1 && tabuleiro[0][j+1]!= -1){                              // if(tabuleiro[0][j] == -1){
			for(i = 0; i < sz; i++){                                                         // for(i = 0; i < sz; i++){
				tabuleiro[i][j] = tabuleiro[i][j+1];                                            // tabuleiro[i][j] = tabuleiro[i][j+1];
				tabuleiro[i][j+1] = -1;                                                         // tabuleiro[i][j+1] = -1;
			}                                                                                // }
      j=-1;                                                                            //
		}                                                                                 // }
	}                                                                                  // }
