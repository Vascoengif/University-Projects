...
FILE_IMAGE_GRAY_A: .asciiz "/home/vasco/Desktop/ASC I/Trabalhofim/lenaA.gray"
FILE_IMAGE_GRAY_B: .asciiz "/home/vasco/Desktop/ASC I/Trabalhofim/lenaB.gray"
...
main:
	...
	beq $v0, 2, MEDIANA
	nop
	beq $v0, 3, SAIR			#se o numero inserido for 3 vai para a label SAIR
	nop
...
FOR:
	...
	div $t0, $s4
	mflo $t0				#guarda o quociente do numero
...
MEDEAN_FILTER:
	...
	li $s2, 511				#linha
	move $s3, $0				#i=0
	la $s4, AUX_BUFFER			#endereço da matriz 3x3
...
FOR_2:
	...
	sb $t8, 0($s4)				#1ª linha, 1ª posiçao
	sb $t7, 1($s4)				#1ª linha, 2ª posiçao
	sb $t6, 2($s4)				#1ª linha, 3ª posiçao
	sb $t2, 3($s4)				#2ª linha, 1ª posiçao
	sb $t0, 4($s4)				#2ª linha, 2ª posiçao
	sb $t1, 5($s4)				#2ª linha, 3ª posiçao
	sb $t3, 6($s4)				#3ª linha, 1ª posiçao
	sb $t4, 7($s4)				#3ª linha, 2ª posiçao
	sb $t5, 8($s4)				#3ª linha, 3ª posiçao
...
END_COMPARACAO:
	lbu $t0, 4($s4)				#guarda no $t0 o que esta no centro da Matriz auxiliar
