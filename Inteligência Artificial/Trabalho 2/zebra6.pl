...
restrict(I,X,J,Y):-restricoes(L), member(frente(X,Y),L), \+ ((I=1, J=4) ;( I=2, J=6);(I=3,J=5)).
restrict(I,X,J,Y):-restricoes(L), member(esq(X,Y),L), \+ (I is J+1; (I=1, J=6)).
restrict(I,X,J,Y):-restricoes(L), member(dir(X,Y),L), \+ (I is J-1; (I=6, J=1)).
restrict(I,X,J,Y):-restricoes(L), member(lado(X,Y),L), \+ ((I is J-1; (I=6, J=1));(I is J+1; (I=1,J=6)) ).
restrict(I,X,J,Y):-restricoes(L), member(cabeceira(X),L), \+ (I=1 ; I=4).
restrict(I,X,J,Y):-restricoes(L), member(cabeceira_esq(X),L), \+ I=1.
restrict(I,X,J,Y):-restricoes(L), member(cabeceira_dir(X),L), \+ I=4.
restrict(I,X,J,Y):- I\=J, X = Y.

restricoes([esq('Manuel','Maria'), frente('Joaquim','Maria'), lado('Joaquim','Matilde'), cabeceira('Joaquim') ]).
